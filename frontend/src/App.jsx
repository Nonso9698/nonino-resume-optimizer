...
import { jsPDF } from 'jspdf';  // NEW: Added for PDF functionality
...
export default function NoninoResumeOptimizer() {
  ...
  // UPDATED: convertTextToParagraphs with all formatting fixes
  const convertTextToParagraphs = (text) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (isFirstLine && trimmedLine.length > 0) {
        ...
        paragraphs.push(new Paragraph({
          children: [new TextRun({ 
            text: trimmedLine,
            bold: true,
            size: 32,  // CHANGED: 16pt (was 28/14pt)
            font: "Calibri"
          })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 20 }  // Tight spacing after name
        }));
        return;
      }
      ...
      if (index === 1) {
        paragraphs.push(new Paragraph({
          children: [new TextRun({ 
            ...
            size: 18,             // CHANGED: 9pt (was 20/10pt)
            font: "Calibri"
          })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 120 }
      ...
      if (inEducationSection && !trimmedLine.startsWith('-')) {
        ...
        // University line (with city and dates)
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-blue-50 to-blue-100 border-2 border-blue-200 rounded-2xl p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 import React, { useState, useEffect } from 'react';
import { FileText, Briefcase, Upload, Loader2, CheckCircle, AlertCircle, History, Sparkles, Eye, RefreshCw } from 'lucide-react';
import { Document, Paragraph, TextRun, Packer, AlignmentType } from 'docx';
import { saveAs } from 'file-saver';
import { jsPDF } from 'jspdf';  // NEW: Added for PDF functionality
  ...
  const downloadAsPDF = async (content, documentType) => {
    try {
      ...
      lines.forEach((line, index) => {
        ...
        // Education section - university lines
        if (inEducationSection && !trimmedLine.startsWith('-')) {
          if (trimmedLine.match(/^(●\s*)?(master|bachelor|associate|phd|diploma|doctor)/i)) {
            doc.setFontSize(10);
